...
#pragma comment(lib, "ksotptraderapi.lib")
#pragma message("Automatically linking with ksotptraderapi.lib")
...
public:
	...
	virtual void OnRspQryInvestorPositionDetail(CKSOTPInvestorPositionDetailField *pInvestorPositionDetail, CKSOTPRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
	{
		...
		if (bIsLast == true)
		{
			...
			// order’s hedge flag 
			ord.HedgeFlag = KS_OTP_HF_UnCovered;
			// price 
			ord.LimitPrice = 0.25; 
			// volume 
	...
	virtual void OnRspQryTradingCode(CKSOTPTradingCodeField *pTradingCode, CKSOTPRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
	{
		...
		{
			// send order insertion request.
			CKSOTPInputNonTradingOrderField ord = {0};
			// broker id 
			strcpy(ord.BrokerID, m_chBrokerID); 
			// investor id
			strcpy(ord.InvestorID, m_chUserID); 
			...
			ord.HedgeFlag = KS_OTP_HF_UnCovered;
			// order’s apply flag 
			ord.ApplyFlag = KS_OTP_APF_OTP;
			m_pUserApi->ReqNonTradingOrderInsert(&ord, m_nRequestID++ );
		}
	}

	// nontrading orderInsert response
	virtual void OnRspNonTradingOrderInsert(CKSOTPInputNonTradingOrderField *pInputOTPOrder, CKSOTPRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
	{
		if (pInputOTPOrder != NULL)
		{
			cout << "非交易报单录入请求响应:"
				...
				<< "非交易报单引用:" << pInputOTPOrder->OTPOrderRef
				...
				<< "备兑标志:" << pInputOTPOrder->HedgeFlag
				<< "申请类型标志:" << pInputOTPOrder->ApplyFlag<< endl;
		}
		cout << endl
			<< "ErrorCode=" << pRspInfo->ErrorID
			<< "ErrorMsg=" << pRspInfo->ErrorMsg
	...
	virtual void OnRtnNonTradingOrder(CKSOTPNonTradingOrderField *pNonTradingOrder)
	{
		cout << "OnRtnNonTradingOrder:"
			...
			<< "非交易报单引用:" << pNonTradingOrder->OTPOrderRef
			...
			<< "本地非交易报单编号:" << pNonTradingOrder->OTPOrderLocalID
			...
			<< "经纪公司报单编号:" << pNonTradingOrder->BrokerOTPOrderSeq << endl;

		// send order insertion request
		CKSOTPQryNonTradingOrderField ord = {0};
		// broker id 
		strcpy(ord.BrokerID, m_chBrokerID); 
		// investor id 
		strcpy(ord.InvestorID, m_chUserID); 
		...
		strcpy(ord.InstrumentID, m_chContract);
		//order reference
		strcpy(ord.OTPOrderSysID, "");  
		// exchange id 
		strcpy(ord.ExchangeID, m_chExchangeID); 

		m_pUserApi->ReqQryNonTradingOrder(&ord, m_nRequestID++ );
	}
	...
	virtual void OnRspQryNonTradingOrder(CKSOTPNonTradingOrderField *pOTPOrder, CKSOTPRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
	{
		if (pOTPOrder != NULL)
		{
			cout << "请求查询非交易报单响应:"
				...
				<< "本地非交易报单编号:" << pOTPOrder->OTPOrderLocalID
				...
				<< "经纪公司报单编号:" << pOTPOrder->BrokerOTPOrderSeq << endl;
			if (bIsLast == true)
			{
				// send order insertion request.
				CKSOTPInputNonTradingOrderActionField ord = {0};
				// broker id 
				strcpy(ord.BrokerID, m_chBrokerID); 
				// investor ID 
				strcpy(ord.InvestorID, m_chUserID); 
				...
				// otporder sysid
				strcpy(ord.OTPOrderSysID,pOTPOrder->OTPOrderSysID); 
				// request id
				ord.RequestID = m_nRequestID;

				m_pUserApi->ReqNonTradingOrderAction(&ord, m_nRequestID++ );
			}
		}
		cout << endl
			<< "ErrorCode=" << pRspInfo->ErrorID
	...
	virtual void OnRspNonTradingOrderAction(CKSOTPInputNonTradingOrderActionField *pInputOTPOrderAction, CKSOTPRspInfoField *pRspInfo, int nRequestID, bool bIsLast) 
	{
		if (pInputOTPOrderAction != NULL)
		{
			cout << "非交易报单操作请求响应:"
				...
				<< "非交易报单引用:" << pInputOTPOrderAction->OTPOrderRef
				...
				<< "非交易报单操作编号:" << pInputOTPOrderAction->OTPOrderSysID
		...
		{
			// send order insertion request.
			CKSOTPQryNonTradingOrderVolumeField ord = {0};
			// broker id 
			strcpy(ord.BrokerID, m_chBrokerID); 
			// investor id
			strcpy(ord.InvestorID, m_chUserID); 
			...
			m_pUserApi->ReqQryNonTradingOrderVolume(&ord, m_nRequestID++ );
		}
	}

	// nontrading order volume response
	virtual void OnRspQryNonTradingOrderVolume(CKSOTPNonTradingOrderVolumeField *pOTPOrderVolume, CKSOTPRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
	{
		if (pOTPOrderVolume != NULL)
		{
			cout << "查询非交易报单量响应:"
				...
				<< "可申请执行量:" << pOTPOrderVolume->OTPVolume
				<< "已申请执行量:" << pOTPOrderVolume->OTPedVolume << endl;
		}
		cout << endl
			<< "ErrorCode=" << pRspInfo->ErrorID
			<< "ErrorMsg=" << pRspInfo->ErrorMsg
	...
	virtual void OnRspQryOTPInsCommRate(CKSOTPOTPInsCommRateField *pOTPInsMarginRate, CKSOTPRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
	{
		...
		{
			// send order insertion request.
			CKSOTPQryOTPInsMarginRateField ord = {0};
			// broker id 
			strcpy(ord.BrokerID, m_chBrokerID); 
			// investor ID 
			strcpy(ord.InvestorID, m_chUserID); 
			...
			m_pUserApi->ReqQryOTPInsMarginRate(&ord, m_nRequestID++ );
		}
	}
	...
	virtual void OnRspQryOTPInsMarginRate(CKSOTPOTPInsMarginRateField *pOTPInsMarginRate, CKSOTPRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
	{
		if (pOTPInsMarginRate != NULL)
		{
			cout << "查询个股期权保证金率请求响应:"
				...
				<< "营业部代码:" << pOTPInsMarginRate->BrokerID
				<< "投资者代码:" << pOTPInsMarginRate->InvestorID
				<< "保证金率:" << pOTPInsMarginRate->MarginRateByMoney
				<< "当前保证金金额:" << pOTPInsMarginRate->CurrMargin << endl;
		}
		cout << endl
			<< "ErrorCode=" << pRspInfo->ErrorID
			<< "ErrorMsg=" << pRspInfo->ErrorMsg
	...
	{
		if (pOTPUnderlyingStockInfo != NULL)
		{
			cout << "查询标的券信息响应:"
				<< "经济公司代码:" << pOTPUnderlyingStockInfo->BrokerID
				<< "客户号:" << pOTPUnderlyingStockInfo->InvestorID
				<< "交易所代码:" << pOTPUnderlyingStockInfo->ExchangeID
				<< "产品代码:" << pOTPUnderlyingStockInfo->ProductID
				<< "昨收盘:" << pOTPUnderlyingStockInfo->PreClosePrice << endl;
		}
		cout << endl
			<< "ErrorCode=" << pRspInfo->ErrorID
			<< "ErrorMsg=" << pRspInfo->ErrorMsg
...
int main ()
{
	...
	strncpy (pSpi ->m_chUserID, "0040107096", sizeof(pSpi ->m_chUserID)-1);
	strncpy (pSpi ->m_chPassword, "1", sizeof(pSpi ->m_chPassword)-1);
