/**
 *@文件  ：Readme_ksotpmarket.txt  测试行情API的源代码说明文件
 *@作者  : xdhu
 *@系统名：API测试程序
 *@公司名：胜科金仕达
 *@修改历史：
 ...
 */
一、文件说明：
testKSOTPMarketAPI文件夹包含如下文件：
源文件   testKSOTPMarketAPI.cpp   event.cpp
头文件   testKSOTPMarketAPI.h     event.h
linux环境makefile文件   makefile
windows环境vc项目文件   testKSOTPMarketAPI.sln，testKSOTPMarketAPI.vcproj
帮助文件		Readme_ksotpmarket.txt, Readme_ksotpmarket_en.txt

二、文件功能：
testKSOTPMarketAPI.h/testKSOTPMarketAPI.cpp  			主要实现测试libksmarketdataapi.so动态库的基本功能
event.h/event.cpp                    				主要封装一个windows/linux兼容的事件例程
makefile 			    				主要实现在linux平台下，测试可执行程序的编译。
testKSOTPMarketAPI.sln，testKSOTPMarketAPI.vcproj		主要实现在windows平台下，测试可执行程序的编译。
...
1，编译说明
   ...
   3）  生成目标
	make 或 make vtype=64r 将在 run 目录下，生成 testKSOTPMarketAPI_64 可执行文件，用于32位测试程序。
        make vtype=32r         将在 run 目录下，生成 testKSOTPMarketAPI_32 可执行文件，用于64位测试程序。
...
2，运行说明
   ...
   1）运行检查
      测试代码的运行，需要确保动态库libksmarketdataapi.so,libkslkc64r.so(32位则是libkslkc32r.so)文件的存在。（发布物包括32位和64位的libksmarketdataapi.so，文件名相同，分别在KSOTPMarketAPI/linux32/和   KSOTPMarketAPI/linux64目录下。而libkslkc32r.so在KSOTPMarketAPI/linux32/目录下，libkslkc64r.so在KSOTPMarketAPI/linux64/目录下）
   ...
   4) 检查环境配置的正确性
	...
	ldd  testKSOTPMarketAPI_32   检查32位测试程序对动态库的加载
	ldd  testKSOTPMarketAPI_64   检查64位测试程序对动态库的加载
      b)查看当前的动态库的版本信息
...
1，编译说明
   ...
   2）如何编译
   通过vs打开testKSOTPMarketAPI.sln文件，编译即可。
   
   3）生成目标
   在run目录下生成testKSOTPMarketAPI.exe可执行文件。

2，运行说明
   确保run目录下有KSMarketDataAPI.dll,cpack.dll,lkcdll.dll
   在run目录下执行testKSOTPMarketAPI.exe即可。
