/**
 *@file   Readme_ksotpmarket_en.txt      English Documentation for source code file of testing MarketData API 
 *@auhor  : xdhu
...
testKSOTPMarketAPI directory includes the following files
source file   :testKSOTPMarketAPI.cpp   event.cpp
head file     :testKSOTPMarketAPI.h     event.h
makefile of Linux enviroment:   makefile
project file of windows enviroment :testKSOTPMarketAPI.sln£¬testKSOTPMarketAPI.vcproj
help doc:       Readme_ksotpmarket.txt , Readme_ksotpmarket_en.txt(English) 

Part Two£ºFiles Function£º
testKSOTPMarketAPI.h/testKSOTPMarketAPI.cpp  			implement the main functions of testing libksmarketdataapi.so dynamic library
...
testKSOTPMarketAPI.sln£¬testKSOTPMarketAPI.vcproj		implement the compile progress of the test demo for windows platform
...
1, Compile Description
   ...
   3£©  Executable Files
	Run "make" or "make vtype=64r" will generate "testKSOTPMarketAPI_64" Executable file in run directory which used to test 64-bit 	libksmarketdataapi.so
        Run "make vtype=32r" will generate "testKSOTPMarketAPI_32" Executable file in run directory which used to test 32-bit libksmarketdataapi.so      
...
2£¬Run Description
   ...
   1£©Run Checking
      First make sure the existence of libksmarketdataapi.so and libkslkc64r.so£¨Release files include both 32 and 64 bit libksmarketdataapi.so,which uses the same file name, divided by the different directory "KSOTPMarketAPI/linux32" and "KSOTPMarketAPI/linux64"¡£ libkslkc32r.so is in KSOTPMarketAPI/linux32 directory, while libkslkc64r.so is in KSOTPMarketAPI/linux64 directory.£©
   ...
   4) Check Correctness of environment configuration
	...
	ldd  testKSOTPMarketAPI_32   : check the load stage of 32-bit test demo 
	ldd  testKSOTPMarketAPI_64   : check the load stage of 64-bit test demo
...
1£¬Compile Description
   ...
   2£©How to Compile
   Use Visual Studio 2005 to open "testKSOTPMarketAPI.sln" file £¬then compile in the IDE.
   
   3£©Executable Files
   After compiling progress , you can find the executable file : testKSOTPMarketAPI.exe in the run directory .

2£¬Run Description
   First make sure run directory has these files: KSMarketDataAPI.dll,cpack.dll,lkcdll.dll
   Then you can execute the testKSOTPMarketAPI.exe in run directory.
   The correctness for running program also needs authorization document "KSInterB2C.lkc" which belongs to Sungard.
   Make sure the running directory has "KSInterB2C.lkc" file .You can connect hot line  for this file if you don't have it .
